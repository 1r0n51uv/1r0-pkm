...
| ~~`1r0-pkm-1.0-b7-contatore-tracker-extra-toast-hk.ipa`~~ | 1.0 (7) | `8184103` | tutto quanto sopra + contatore centrale acqua/caffeina, alimento extra in un pasto già mangiato + rimozione per voce, toast quando Salute ha un problema (ADR-0037) | **rimossa**, superata dalla riga sotto. |
| `1r0-pkm-1.0-b8-fix-bundle-id-healthkit.ipa` | 1.0 (8) | `c3ecc37` | tutto quanto sopra + bundle id cambiato `dev.1r0.pkm` → `dev.1r0.pkm2` per forzare una nuova registrazione App ID e sbloccare l'entitlement HealthKit su sideload (ADR-0038) | beta. **Richiede disinstallare la vecchia app prima di reinstallare** (bundle id diverso = nuova app agli occhi di SideStore/iOS; dati offline non ancora sincronizzati andrebbero persi, il resto si ripesca dal backend). Da confermare sul device se sblocca davvero il prompt Salute. Backend HTTP (`100.31.154.129`), niente HTTPS. Punta al database di **produzione** per default (switch in Impostazioni per quello di sviluppo). |
